...
def find_api_key(skill_dir: Optional[Path] = None) -> Optional[str]:
    ...
    # Step 3: Check ./.OpenCode/.env
    claude_env = project_dir / '.OpenCode' / '.env'
    ...
    # Step 4: Check ./.OpenCode/skills/.env
    claude_skills_env = project_dir / '.OpenCode' / 'skills' / '.env'
...
def find_env_var(var_name: str, skill_dir: Optional[Path] = None) -> Optional[str]:
    ...
    env_files = [
        project_dir / '.env',
        project_dir / '.OpenCode' / '.env',
        project_dir / '.OpenCode' / 'skills' / '.env',
        skill_dir / '.env'
    ]
...
def get_api_key_or_exit(skill_dir: Optional[Path] = None) -> str:
    ...
    if not api_key:
        ...
        print("\n3ï¸âƒ£  .OpenCode/.env file:", file=sys.stderr)
        print(f"   echo 'GEMINI_API_KEY=your-api-key' > {project_dir}/.OpenCode/.env", file=sys.stderr)

        print("\n4ï¸âƒ£  .OpenCode/skills/.env file (shared across all Gemini skills):", file=sys.stderr)
        print(f"   echo 'GEMINI_API_KEY=your-api-key' > {project_dir}/.OpenCode/skills/.env", file=sys.stderr)
