...
3. **.opencode/.env file**
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > .opencode/.env
   ```

4. **.opencode/skills/.env file** (shared across all Gemini skills)
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > .opencode/skills/.env
   ```

5. **Skill directory `.env` file**
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > .opencode/skills/your-skill/.env
   ```
